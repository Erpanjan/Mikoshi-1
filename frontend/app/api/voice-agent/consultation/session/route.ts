...
export async function POST() {
  ...
  const endpoint = `${advisorServiceUrl}/advisor/api/v1/consultation-voice/signed-url`;
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          error: payload?.error ?? 'Failed to create consultation voice session',
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Consultation voice session request failed',
